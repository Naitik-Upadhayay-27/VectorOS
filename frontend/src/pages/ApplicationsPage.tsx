...
interface App {
  ...
  jobUrl?: string; status: Status; matchScore?: number
  appliedAt?: string; createdAt: string
}

function AddModal({ onClose, onAdd }: { onClose: () => void; onAdd: (a: Partial<App>) => void }) {
  const [form, setForm] = useState({ jobTitle: '', company: '', location: '', jobUrl: '', status: 'saved' as Status })
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl p-6 w-[440px] mx-4">
        ...
        <div className="space-y-3">
          {([['jobTitle','Job Title','e.g. Frontend Engineer'],['company','Company','e.g. Google'],['location','Location','e.g. Remote'],['jobUrl','Job URL','https://...']] as [string,string,string][]).map(([k,l,p]) => (
            <div key={k}>
              ...
              <input value={(form as any)[k]} onChange={e => s(k, e.target.value)} placeholder={p}
            ...
            <div className="flex gap-2 flex-wrap">
              {(Object.keys(STATUS_CONFIG) as Status[]).map(st => (
                <button key={st} onClick={() => s('status', st)}
                  className={`px-3 py-1.5 text-xs font-semibold rounded-lg border capitalize transition-all ${form.status === st ? 'bg-purple-600 text-white border-purple-600' : 'border-gray-200 text-gray-600'}`}>
        ...
        <div className="flex gap-3 mt-5">
          ...
          <button onClick={() => { onAdd(form); onClose() }} disabled={!form.jobTitle || !form.company}
...
export default function ApplicationsPage() {
  ...
  useEffect(() => {
    apiFetch(`${API_BASE}/api/applications`).then(r => r.json())
      .then(d => setApps(d.applications ?? []))
      .finally(() => setLoading(false))
  }, [])

  const addApp = async (form: Partial<App>) => {
    const res = await apiFetch(`${API_BASE}/api/applications`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) })
                      ...
                      {items.map(app => (
                        <div key={app._id} className="bg-white rounded-xl p-3 border border-gray-100 shadow-sm hover:shadow-md transition-shadow group">
                          ...
                          <div className="flex items-center justify-between mt-2">
                            ...
                            {app.matchScore && <span className={`text-[10px] font-bold ${app.matchScore >= 80 ? 'text-green-600' : 'text-blue-600'}`}>{app.matchScore}%</span>}
                          </div>
