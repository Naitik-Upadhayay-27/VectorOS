...
const ResumeTemplate6 = ({ data }: ResumeTemplate6Props) => {
  ...
  return (
    <div className="w-[850px] min-h-[1100px] bg-white overflow-hidden" 
      ...
      {data.summary && (
        <>
          <div style={{ fontSize: '8pt', fontWeight: 600, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '3px', marginBottom: '8px' }}>
            About
          </div>
      ...
      {data.experience && data.experience.length > 0 && (
        <>
          <div style={{ fontSize: '8pt', fontWeight: 600, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '3px', marginBottom: '15px' }}>
            Experience
          </div>
            ...
            <div key={edu.id} style={{ marginBottom: '15px' }}>
              ...
              {edu.gpa && (
                <div style={{ fontSize: '8pt', color: '#9ca3af' }}>
                  GPA: <EditableText value={edu.gpa} onSave={v => setEdu(edu.id, 'gpa', v)} />
                </div>
              )}
              {edu.honors && (
                <div style={{ fontSize: '8pt', color: '#9ca3af', fontStyle: 'italic' }}>
                  {edu.honors}
                </div>
              )}
            </div>
            ...
            <div key={section.id}>
              ...
              {section.items.map((item) => (
                <div key={item.id} style={{ marginBottom: '8px' }}>
                  <div style={{ fontSize: '9pt', fontWeight: 600, color: '#374151' }}>
                    {item.title}
                    {item.date && (
                      <span style={{ fontSize: '8pt', color: '#9ca3af', fontWeight: 'normal', marginLeft: '8px' }}>
                        ({item.date})
                      </span>
                    )}
                  </div>
                  {item.subtitle && (
                    <div style={{ fontSize: '8pt', color: '#6b7280' }}>{item.subtitle}</div>
                  )}
                  {item.description && item.description.length > 0 && (
                    <ul style={{ paddingLeft: '0', listStyle: 'none', margin: '5px 0 0 0' }}>
                      {item.description.map((desc, idx) => (
                        <li key={idx} style={{ marginBottom: '3px', fontSize: '8pt', color: '#4b5563', paddingLeft: '12px', position: 'relative' }}>
                          <span style={{ position: 'absolute', left: '0', color: '#d1d5db' }}>—</span>
                          {desc}
                        </li>
                      ))}
