...
const ResumeTemplate23 = ({ data }: { data: TemplateResumeData }) => {
  ...
  const sectionRenderers: Record<string, () => React.ReactNode> = {
    summary: () => data.summary ? (
      <div key="summary" style={{ padding: '24px 48px 0' }}>
      ...
      <div key="projects" style={{ marginBottom: 24 }}>
        ...
        {data.projects.map((proj: any) => (
          <div key={proj.id} style={{ marginBottom: 12 }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: '#111', margin: '8px 0 2px' }}>{proj.name}</p>
            <p style={{ fontSize: 10.5, color: '#555', margin: 0, lineHeight: 1.5 }}>{Array.isArray(proj.description) ? proj.description[0] : proj.description}</p>
          </div>
        ))}
      </div>
    ) : null,
  ...
  const leftSections = ['education', 'experience', 'projects']
  const rightSections = ['skills', 'languages']
  ...
  return (
    <div style={{ fontFamily, fontSize: `${fontSize}pt`, width: 794, minHeight: 1123, background: '#fff' }}>
      ...
      {/* ── About Me (Optional, always at top if exists) ── */}
      {sectionRenderers.summary()}
      ...
      <div style={{ display: 'flex', padding: '24px 48px 40px', gap: 32 }}>
        ...
        <div style={{ width: 200, flexShrink: 0 }}>
            ...
            {/* Contact is fixed at top of right column */}
...
function SectionHeading({ children }: { children: React.ReactNode }) {
  return (
    <div className="resume-section-head" style={{ borderBottom: '1.5px solid #111', paddingBottom: 6 }}>
