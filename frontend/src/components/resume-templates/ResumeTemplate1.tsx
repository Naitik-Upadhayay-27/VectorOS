...
const ResumeTemplate1 = ({ data }: { data: TemplateResumeData }) => {
  ...
  const setExpBullet = (id: string, idx: number, val: string) => {
    ...
    if (!exp) return
    const desc = [...(exp.description ?? [])]; desc[idx] = val
  ...
  const setEduBullet = (id: string, idx: number, val: string) => {
    ...
    if (!edu) return
    const desc = [...(edu.description ?? [])]; desc[idx] = val
  ...
  const setProjBullet = (id: string, idx: number, val: string) => {
    ...
    desc[idx] = val; store.updateProject(id, { description: desc })
  }

  const sectionRenderers: Record<string, () => React.ReactNode> = {
    summary: () => data.summary ? (
      <section key="summary" className="mb-6">
        <h2 className="text-[16px] font-bold uppercase tracking-wider text-gray-900 mb-3 border-b-2 border-gray-400 pb-1">Professional Summary</h2>
    ...
    experience: () => data.experience?.length > 0 ? (
      <section key="experience" className="mb-6">
        <h2 className="text-[16px] font-bold uppercase tracking-wider text-gray-900 mb-3 border-b-2 border-gray-400 pb-1">Professional Experience</h2>
        {data.experience.map((exp) => (
          <div key={exp.id} className="mb-5">
            <div className="flex justify-between items-start mb-2">
              <div>
                <h3 className="text-[15px] font-bold text-gray-900"><EditableText value={exp.title} onSave={v => setExp(exp.id, 'title', v)} /></h3>
            ...
            {exp.description?.length > 0 && (
              <ul className="list-disc list-outside ml-4 text-[12px] text-gray-700 leading-relaxed space-y-1">
                {exp.description.map((desc, i) => <li key={i}><EditableText value={desc} onSave={v => setExpBullet(exp.id, i, v)} multiline /></li>)}
              </ul>
            )}
          </div>
        ))}
    ...
    education: () => data.education?.length > 0 ? (
      <section key="education" className="mb-6">
        <h2 className="text-[16px] font-bold uppercase tracking-wider text-gray-900 mb-3 border-b-2 border-gray-400 pb-1">Education</h2>
        {data.education.map((edu) => (
          <div key={edu.id} className="mb-4">
            <div className="flex justify-between items-start mb-2">
              <div>
                <h3 className="text-[14px] font-bold text-gray-900"><EditableText value={edu.degree} onSave={v => setEdu(edu.id, 'degree', v)} /></h3>
              ...
              <p className="text-[12px] text-gray-600"><EditableText value={edu.graduationDate} onSave={v => setEdu(edu.id, 'graduationDate', v)} /></p>
            </div>
            {edu.description?.length > 0 && (
              <ul className="list-disc list-outside ml-4 text-[12px] text-gray-700 leading-relaxed space-y-1">
                {edu.description.map((desc, i) => <li key={i}><EditableText value={desc} onSave={v => setEduBullet(edu.id, i, v)} multiline /></li>)}
              </ul>
            )}
          </div>
        ))}
    ...
    skills: () => data.skills?.length > 0 ? (
      <section key="skills" className="mb-6">
        <h2 className="text-[16px] font-bold uppercase tracking-wider text-gray-900 mb-3 border-b-2 border-gray-400 pb-1">Technical Skills</h2>
        {data.skills.map((cat) => (
          <div key={cat.id} className="mb-3">
            <h3 className="text-[13px] font-bold text-gray-900 mb-1"><EditableText value={cat.category} onSave={v => setSkill(cat.id, 'category', v)} /></h3>
    ...
    projects: () => data.projects?.length > 0 ? (
      <section key="projects" className="mb-6">
        <h2 className="text-[16px] font-bold uppercase tracking-wider text-gray-900 mb-3 border-b-2 border-gray-400 pb-1">Projects</h2>
        {data.projects.map((proj) => (
          <div key={proj.id} className="mb-4">
            <div className="flex justify-between items-start mb-2">
              <div>
                <h3 className="text-[14px] font-bold text-gray-900"><EditableText value={proj.name} onSave={v => setProj(proj.id, 'name', v)} /></h3>
            ...
            {proj.description && (
              Array.isArray(proj.description) ? proj.description.length > 0 && (
                <ul className="list-disc list-outside ml-4 text-[12px] text-gray-700 leading-relaxed space-y-1">
                  {proj.description.map((desc, i) => <li key={i}><EditableText value={desc} onSave={v => setProjBullet(proj.id, i, v)} multiline /></li>)}
                </ul>
              ) : <p className="text-[12px] text-gray-700 mt-1"><EditableText value={proj.description} onSave={v => setProj(proj.id, 'description', v)} multiline /></p>
            )}
    ...
    certificates: () => data.certificates?.length > 0 ? (
      <section key="certificates" className="mb-6">
        <h2 className="text-[16px] font-bold uppercase tracking-wider text-gray-900 mb-3 border-b-2 border-gray-400 pb-1">Certifications</h2>
    ...
    awards: () => data.awards?.length > 0 ? (
      <section key="awards" className="mb-6">
        <h2 className="text-[16px] font-bold uppercase tracking-wider text-gray-900 mb-3 border-b-2 border-gray-400 pb-1">Awards &amp; Accomplishments</h2>
    ...
    languages: () => data.languages?.length > 0 ? (
      <section key="languages" className="mb-6">
        <h2 className="text-[16px] font-bold uppercase tracking-wider text-gray-900 mb-3 border-b-2 border-gray-400 pb-1">Languages</h2>
    ...
    volunteer: () => data.volunteer?.length > 0 ? (
      <section key="volunteer" className="mb-6">
        <h2 className="text-[16px] font-bold uppercase tracking-wider text-gray-900 mb-3 border-b-2 border-gray-400 pb-1">Volunteering &amp; Leadership</h2>
        {data.volunteer.map((v) => (
          <div key={v.id} className="mb-3">
            <div className="flex justify-between items-start">
              <div>
                <h3 className="text-[13px] font-bold text-gray-900">{v.role}</h3>
                <p className="text-[12px] text-gray-600">{v.organization}</p>
              </div>
              <p className="text-[12px] text-gray-600">{v.startDate} – {v.endDate}</p>
            </div>
            {v.description && <p className="text-[12px] text-gray-700 mt-1">{v.description}</p>}
          </div>
        ))}
      </section>
    ) : null,
      ...
      <header className="pb-4 border-b-2 border-gray-400 mb-6">
        <div className="flex items-center gap-6 mb-3">
          {data.personalInfo?.image && (
            <img src={data.personalInfo.image} alt="" className="w-20 h-20 rounded-full object-cover border-2 border-gray-300 shrink-0" />
          )}
        ...
        <div className="flex flex-wrap gap-x-6 gap-y-2 text-[12px] text-gray-700">
          {data.personalInfo?.contact?.email && <span className="flex items-center gap-2"><Mail className="w-4 h-4 text-blue-600" /><EditableText value={data.personalInfo.contact.email} onSave={v => setContact('email', v)} /></span>}
          {data.personalInfo?.contact?.phone && <span className="flex items-center gap-2"><Phone className="w-4 h-4 text-blue-600" /><EditableText value={data.personalInfo.contact.phone} onSave={v => setContact('phone', v)} /></span>}
          {data.personalInfo?.contact?.location && <span className="flex items-center gap-2"><MapPin className="w-4 h-4 text-blue-600" /><EditableText value={data.personalInfo.contact.location} onSave={v => setContact('location', v)} /></span>}
          {data.personalInfo?.contact?.linkedin && <span className="flex items-center gap-2"><Linkedin className="w-4 h-4 text-blue-600" /><EditableText value={data.personalInfo.contact.linkedin} onSave={v => setContact('linkedin', v)} /></span>}
          {data.personalInfo?.contact?.github && <span className="flex items-center gap-2"><Github className="w-4 h-4 text-blue-600" /><EditableText value={data.personalInfo.contact.github} onSave={v => setContact('github', v)} /></span>}
        </div>
      </header>
