        ...
        {data.volunteer.map((vol) => (
          <div key={vol.id} style={{ marginBottom: '8px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                <div style={{ fontWeight: 600, fontSize: '9pt', color: '#1a1a2e' }}>{vol.role}</div>
                <div style={{ fontSize: '8.5pt', color: '#4a5568' }}>{vol.organization}</div>
              </div>
              <div style={{ fontSize: '8pt', color: '#718096' }}>{vol.startDate} – {vol.endDate}</div>
            </div>
            {vol.description && <div style={{ fontSize: '8pt', color: '#4a5568', marginTop: '2px' }}>{vol.description}</div>}
          </div>
        ))}
      </div>
    ) : null,
