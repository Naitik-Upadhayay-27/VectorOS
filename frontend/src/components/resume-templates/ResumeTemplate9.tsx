  ...
  return (
    <div className="w-[850px] min-h-[1100px] bg-white overflow-hidden flex"
          ...
          <div style={{ marginBottom: '14px' }}>
            ...
            {data.skills.map((skillGroup) => (
              <div key={skillGroup.id} style={{ marginBottom: '8px' }}>
                <div style={{ fontSize: '8pt', fontWeight: 600, color: '#f1f5f9', marginBottom: '2px' }}>
                  <EditableText value={skillGroup.category} onSave={v => setSkill(skillGroup.id, 'category', v)} />
                </div>
                <div style={{ fontSize: '7pt', color: '#94a3b8', lineHeight: '1.5' }}>
        ...
        {data.languages && data.languages.length > 0 && (
          <div style={{ marginBottom: '14px' }}>
            ...
            {data.languages.map((lang) => (
              <div key={lang.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '7.5pt', marginBottom: '3px' }}>
          ...
          <div style={{ marginBottom: '14px' }}>
            ...
            {data.certificates.map((cert) => (
              <div key={cert.id} style={{ marginBottom: '6px' }}>
                <div style={{ fontSize: '7.5pt', fontWeight: 600, color: '#f1f5f9' }}>{cert.name}</div>
                <div style={{ fontSize: '7pt', color: '#64748b' }}>{cert.issuer}, {cert.date}</div>
              </div>
            ))}
          </div>
        )}
      ...
      <div style={{ flex: 1, padding: '0.4in 0.5in' }}>
        ...
        {data.summary && (
          <div>
            <div style={{ fontSize: '11pt', fontWeight: 700, color: '#0f172a', marginBottom: '8px', marginTop: '0', paddingBottom: '3px', borderBottom: `2px solid ${accentColor}` }}>
              Profile
            </div>
